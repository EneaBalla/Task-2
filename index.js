...
function filterByRegion() {
  ...
  fetch("./data.json")
    .then((res) => res.json())
    .then((data) => {
      const filteredData = data.filter((element) => element.region === region);
      clearResults();
      displayResults(filteredData);
    });
}
      ...
      .then((data) => {
        ...
        if (country) {
          ...
          document.getElementById(
            "nativename"
          ).innerHTML = `<b>Native Name: </b> ${country.nativeName}`;
          document.getElementById(
            "population"
          ...
          document.getElementById(
            "subregion"
          ).innerHTML = `<b>Sub Region: </b> ${country.region}`;
          document.getElementById(
            "capital"
          ).innerHTML = `<b>Capital: </b> ${country.capital}`;
          document.getElementById(
            "tldomain"
          ).innerHTML = `<b>Top Level Domain: </b> ${country.topLevelDomain.join(
            ", "
          )}`;
          document.getElementById(
            "currencies"
          ).innerHTML = `<b>Currencies: </b> ${country.currencies
            .map((curr) => curr.name)
            .join(", ")}`;
          document.getElementById(
            "languages"
          ).innerHTML = `<b>Languages: </b> ${country.languages
            .map((lang) => lang.name)
            .join(", ")}`;
          document.getElementById(
            "border"
          ).innerHTML = `<b>Border Countries: </b>${country.borders}`;
        }
      });
  }
}
...
function toggleDarkMode() {
    if (!isToggled) {
      isToggled = true;
      document.getElementById("shtyp").innerHTML =
